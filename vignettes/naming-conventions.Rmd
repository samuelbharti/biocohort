...
While myceliumr does not enforce specific sample ID formats, adopt a consistent project-wide convention:

- **WES Tumor**: `WES_T{subject}` or `DNA_T{number}` (e.g., "WES_T101", "DNA_T001")
- **WES Normal**: `WES_N{subject}` or `DNA_N{number}` (e.g., "WES_N101", "DNA_N001")
- **snRNA-seq**: `SN{subject}_{library}` or `RNA_{subject}_{rep}` (e.g., "SN101_1", "RNA_T101_2")
- **Pair ID**: `{tumor_sample_id}__{normal_sample_id}` (e.g., "WES_T101__WES_N101", "DNA_T001__DNA_N001")

## Object Names

### R Objects and Variables

- **Study objects**: PascalCase, descriptive (e.g., `my_study`, `nf1_study`) or explicitly named `study` in examples.
- **Cohort objects**: PascalCase or snake_case (e.g., `cohort`, `nf1_cohort`, `study_cohort`).
...
- **Input data**: Describe clearly (e.g., `meta_rats`, `subject_tbl`, `sample_map`)
- **Flags**: Prefix with `is_` or `has_` (e.g., `is_valid`, `has_missing`)
- **Counts**: Prefix with `n_` (e.g., `n_subjects`, `n_samples`)
- **Logical conditions**: `strict`, `verbose`, `allow_*` (e.g., `allow_rna_duplicates`)
...
- Use sentence case (e.g., "Getting started", "Glossary", "Naming conventions")
...
- [ ] Assay column values are lowercase with underscores (e.g., "dna_wes", "rna_snrna")
