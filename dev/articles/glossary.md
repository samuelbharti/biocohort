...
| **Subject Object** | An S7 instance of the Subject class encapsulating a single subject’s metadata. Built on demand from a Cohort with `subject(cohort, id)`, or directly with [`subject_new()`](https://www.samuelbharti.com/bioroster/reference/subject_new.md). A Cohort stores subjects as rows of `subject_tbl`, not as objects. |
